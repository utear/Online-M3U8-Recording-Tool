...
### 2024-05-22
- 彻底重构用户登录模块，解决CORS跨域问题
- 简化CORS中间件，采用更直接的方式处理跨域请求
- 移除重复的CORS配置，减少相互干扰
- 添加SIMPLE_CORS环境变量选项，允许在遇到问题时快速切换到简化模式
- 重构前端登录和注册组件，采用更简洁的请求方式
- 简化axios配置，移除不必要的复杂性
- 增强后端路由处理，添加详细的日志记录
- 更新setup.js脚本，增强CORS配置选项
- 统一路由处理，移除server.js中的重复路由
...
- 增强OPTIONS预检请求处理，添加Access-Control-Max-Age头部减少预检请求频率
- 重构前端登录和注册组件，添加多重备用请求机制：
  * 首先尝试使用自定义axios实例进行请求
  * 如果axios失败，尝试使用fetch API作为备用
  * 最后尝试使用原生XMLHttpRequest作为最终备用方案
- 优化后端服务器CORS配置，同时使用自定义中间件和cors库提供双重保障
- 完善请求日志记录，添加详细的请求和响应信息便于调试
- 增强错误处理机制，提供更清晰的错误提示和故障恢复能力

### 2024-05-19
- 彻底重构CORS跨域处理机制，创建专用CORS中间件模块
- 采用更宽松的CORS配置，允许所有来源访问，解决生产环境中的跨域问题
- 优化OPTIONS预检请求处理，添加全局OPTIONS请求处理器
- 重构前端请求机制，同时支持axios和fetch API两种方式
- 增强前端错误处理，添加详细的日志输出和备用请求机制
- 优化登录和注册组件，使用完整URL进行请求避免路径问题
- 添加详细的请求日志，便于调试跨域问题
...
- 优化了OPTIONS预检请求处理，添加了详细日志输出便于调试
- 扩展了allowedHeaders配置，增加了'Accept'和'Origin'头部支持
- 修改了后端监听地址为0.0.0.0，确保可以接受所有外部连接
- 完善了CORS域名配置，增加了WebSocket协议支持
- 优化了前端环境变量配置，确保使用正确的域名访问后端API
- 更新了setup.js一键部署脚本，增强了CORS配置的生成逻辑
- 完善了环境变量文件的注释，提供了更清晰的配置指导
...
- 优化CORS域名输入体验，用户只需输入域名部分即可自动生成不同端口的完整地址
- 完善了前端环境变量配置，自动将自定义域名添加到VITE_ALLOWED_HOSTS
- 增加了API和WebSocket地址选择功能，支持在部署时直接配置生产环境域名
- 修复了CORS配置中URL重复的问题，优化了配置生成逻辑
