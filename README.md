...
#### 优化功能
1. **文件下载功能重构**:
   - 改用浏览器原生下载方式，解决大文件（>1GB）下载时的内存问题
   - 优化认证机制，支持通过URL参数传递token
   - 从数据库获取任务信息，确保服务器重启后仍能下载
   - 改进权限验证，确保用户只能下载自己的任务文件
2. **新增 token 自动刷新机制**，解决长时间未操作导致需要重新登录的问题
  - 后端新增 `/api/auth/refresh-token` 接口
  - 前端自动在 token 即将过期时刷新，提升用户体验
